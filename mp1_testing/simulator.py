...
class Simulator:
    ...
    def set_spawn_points(self, initial_ego_state, initial_lead_state):
        ...
        self.ego_vehicle_spawn_point = carla.Transform(
            ...
            carla.Rotation(pitch=0.000000, yaw=ego_yaw + 180, roll=0.000000),
        )
    ...
    def _set_ado_control(self, ado_actions):
        ...
        for action in ado_actions:
            self.ado_throttle.append(action['throttle'])
            self.ado_brake.append(action['brake'])
            self.ado_steer.append(action['steer'])
